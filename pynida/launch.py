...
from pynida.simple_functions import linear, sneddon, Convert, eab, parse_corr_data, parse_hys_data, get_n_frame, \
    parse_video_data, parse_old_data
...
class mywindow(QtWidgets.QMainWindow):
    ...
    def plot_converted(self):
        ...
        plt.close()
        self.ui.label_finimg.setPixmap(
            QPixmap(self.ui.lineEdit_27.text()[:-5] + '_visall.png').scaled(self.ui.label_finimg.size(),
    ...
    def btnClicked_AreasConv(self):
        ...
        try:
            ...
            self.a_frame_time = (df['frame_number'].values + float(self.ui.lineEdit_shift.text()))*float(
    ...
    def btnClicked_ContoursConv(self):
        ...
        try:
            ...
            while i < len(c_data_desc):
                ...
                i += 1
            self.c_vol = (c_vol_top + c_vol_bot)/2./c_scale**3*float(
                self.ui.lineEdit_pnm2.text())**3/10**9  # vol in mkm^3
...
#			'''
        ...
        if todo:
            try:
                for q in [self.ui.tab1, self.ui.tab3, self.ui.tab4, self.ui.tab_3, self.ui.lineEdit_AreaScale,
                          ...
                          self.ui.lineEdit_TestFrN, self.ui.lineEdit_sm_blur_val, self.ui.pushButton_10,
                          ...
                          self.ui.pushButton_a1_b, self.ui.pushButton_a1_t, self.ui.pushButton_a1_l,
                          self.ui.pushButton_a1_r,
                          self.ui.pushButton_a2_b, self.ui.pushButton_a2_t, self.ui.pushButton_a2_l,
                          self.ui.pushButton_a2_r,
                          self.ui.pushButton_crop_2, self.ui.lineEdit_30, self.ui.checkBox_Rotate_2,
                          self.ui.RotationAngle_2]:
                    q.setEnabled(False)
                ...
                while i <= last_fr_num:
                    ...
                    if i < first_fr_num:
                        ...
                        progress_area.setValue(
                            int((i - first_fr_num)/(last_fr_num - first_fr_num + 1)*100))  # progressbar
                        try:
                            # if 1:
                ...
                # change column order
                df_data_areas = df_data_areas[
                    ['substr_angle', 'tip_sample_contact_length', 'dx_tip_sample', 'sample_surface_contact_length',
                     'dx_sample_surface', 'mid_projection_x', 'mid_projection_y', 'len_mid', 'square_x', 'square_y',
                     'ellipse_major', 'ellipse_minor', 'area', 'orientation']]
                ...
                while i <= last_fr_num:
                    ...
                    if i < first_fr_num:
                        ...
                        self.ui.progressBar_3.setValue(
                            (i - first_fr_num)/(last_fr_num - first_fr_num + 1)*100)  # progressbar
                        try:
                            _, test_frame = cap.read()
                            test_frame = cv2.cvtColor(test_frame, cv2.COLOR_BGR2GRAY)
                            #######
    ...
    def btnClicked_TestCall(self):
        ...
        if todo:
            ...
            try:
                a = ['imgIni', 'imgDist1', 'imgMark1', 'imgRw1', 'imgCut', 'imgDist2', 'imgMark2', 'imgRw2',
                     'imgParticle', 'imgFin']
    ...
    def btnClicked_TestCall_contours(self):
        ...
        if todo:
            try:
                ...
                # '''
                try:
                    self.ui.label_imgIni_2.setPixmap(
                        QPixmap(self.savepath + '_c_initial.png').scaled(self.ui.label_imgIni_2.size(),
                                                                         QtCore.Qt.KeepAspectRatio,
                                                                         QtCore.Qt.SmoothTransformation))
                    self.ui.label_imgIni_2.show()
                    self.ui.label_imgRw2_2.setPixmap(
                        QPixmap(self.savepath + '_contours2.png').scaled(self.ui.label_imgRw2_2.size(),
                                                                         QtCore.Qt.KeepAspectRatio,
                                                                         QtCore.Qt.SmoothTransformation))
...
#		allow_contour = self.ui.checkBox_Cont.isChecked()
    ...
    def btnClicked_import_hysdata(self):
        ...
        self.hys_filename, _ = QFileDialog.getOpenFileName(self, "Select hysitron data file", os.getcwd(),
                                                           "txt files (*.hystxt);;txt files (*.txt);;All Files (*)",
                                                           options=options)
    ...
    def btnClicked_import_olddata(self):
        ...
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        self.old_filename, _ = QFileDialog.getOpenFileName(self, "Select the data file", os.getcwd(),
    ...
    def btnClicked_import_videodata(self):
        ...
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        self.video_filename, _ = QFileDialog.getOpenFileName(self, "Select video data file", os.getcwd(),
                                                             options=options)
    ...
    def btnClicked_import_corrdata(self):
        ...
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        self.corr_filename, _ = QFileDialog.getOpenFileName(self, "Select video data file", os.getcwd(),
                                                            options=options)
    ...
    def btnClicked_AreasOpen(self):
        ...
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        self.area_data_import, _ = QFileDialog.getOpenFileName(self, "Select areas data file", os.getcwd(),
                                                               options=options)
    ...
    def btnClicked_ContoursOpen(self):
        ...
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        self.contours_data_import, _ = QFileDialog.getOpenFileName(self, "Select contours data file", os.getcwd(),
                                                                   options=options)
    ...
    def btnClicked_selectfile(self):
        ...
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        fileName, _ = QFileDialog.getOpenFileName(self, "Select a video file", os.getcwd(),
        ...
        if fileName != '':
            ...
            try:
                ...
                self.savepath = os.path.join(self.savedir, self.name_of_videofile[self.name_of_videofile.rfind(
                    '/') + 1: self.name_of_videofile.rfind('.')])
        ...
        if todo:
            try:  # if mag and scale were chosen
                ...
                self.ui.label_10.setPixmap(
                    QPixmap(self.savepath + '_mv0.png').scaled(self.ui.label_10.size(), QtCore.Qt.KeepAspectRatio,
                                                               QtCore.Qt.SmoothTransformation))
                self.ui.label_10.show()

                self.ui.label_13.setPixmap(
                    QPixmap(self.savepath + '_mv1.png').scaled(self.ui.label_13.size(), QtCore.Qt.KeepAspectRatio,
                                                               QtCore.Qt.SmoothTransformation))
    ...
    def btnClicked_P3(self):
        ### parse data
        # video
        self.time_v, self.disp_v, self.pnmi = parse_video_data(self.video_filename, self.separator,
                                                               self.ui.corr_bckgr.isChecked())
        self.time_v, self.disp_v = self.time_v - self.time_v[0], self.disp_v - self.disp_v[
            0]  # Only relative coordinates matter
        # Hys
    ...
    def btnClicked_P7(self):
        ...
        self.ui.label_37.setPixmap(
            QPixmap(savepath + '_corrected.png').scaled(self.ui.label_37.size(), QtCore.Qt.KeepAspectRatio,
                                                        QtCore.Qt.SmoothTransformation))
            ...
            if todo:
                ...
                try:
                    ...
                    QMessageBox.about(self, "Error",
                                      "Some problem detected with the selected frame.\nPlease, select another one")
        else:
            QMessageBox.about(self, "Alarm!", "Select a video file!")
...
def tracker(name_of_videofile, coords_list, crop, frames_n, rotate=False,
            rotation_angle=0):  # last frames are empty and cause error
...
def toggle_selector_crop(event):
    ...
    if event.key in ['enter']:
        ...
        crop_x0, crop_x1, crop_y0, crop_y1 = max(0, int(cr_x1)), min(init_img_width, int(cr_x2)), max(0,
                                                                                                      int(cr_y1)), min(
            init_img_height, int(cr_y2))
